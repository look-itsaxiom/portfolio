...
export default function ImpactListPage() {
  const { data, loading, error, refetch } = useApi<Entry[]>("/api/impact")
  const navigate = useNavigate()
  const [deleting, setDeleting] = useState<string | null>(null)

  async function handleDelete() {
    if (!deleting) return
    await apiDelete(`/api/impact/${deleting}`)
    setDeleting(null)
    refetch()
  }
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-zinc-100">Impact</h1>
        <button
          ...
          <tbody>
            {data.map((entry) => (
              <tr
                ...
                onClick={() => navigate(`/impact/${entry.slug}`)}
              >
